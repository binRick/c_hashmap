...
int utest_main(int argc, const char *const argv[]) {
  ...
  for (index = 1; index < UTEST_CAST(size_t, argc); index++) {
    ...
    if (0 == UTEST_STRNCMP(argv[index], help_str, strlen(help_str))) {
      printf("include/hashmap.h - the single file unit testing solution for C/C++!\n"
...
/*
   define a main() function to call into include/hashmap.h and start executing tests! A
